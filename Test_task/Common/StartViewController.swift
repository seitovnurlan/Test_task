...
//            make.trailing.equalToSuperview().inset(75)
    ...
    private func initAction() {
        ...
        securyButton.addTarget(self,
                        action: #selector(goToClick(_ :)),
                         for: .touchUpInside)
        forgotPaswButton.addTarget(self,
                        action: #selector(goForgClick(_ :)),
                        for: .touchUpInside)
        createNowButton.addTarget(self,
                        action: #selector(goCreatNow( _:)),
                        for: .touchUpInside)
    ...
    @objc func goForgClick(_ sender: UIButton) {
        let fourVC = FourthViewController()
        fourVC.nameEmail = emailTextFil.text ?? "empty"
        self.navigationController?.pushViewController(fourVC, animated: true)
    }
    @objc func goCreatNow(_ sender: UIButton) {
        let fifthVC = FifthViewController()
        //fifthVC.nameValue = emailTextFil.text ?? "empty"
